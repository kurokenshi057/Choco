...
public class Program
{
    ...
    private static void FinaliserCommande(Acheteur acheteur)
    {
        ...
        // Créez un répertoire avec le nom de l'utilisateur s'il n'existe pas
        ...
        // Enregistrer les articles achetés
        EnregistrerArticlesAchetes();
    ...
    private static string ConstruireFacture(Acheteur acheteur, float prixTotal)
    {
        ...
        // Filtrer les articles du panier de l'acheteur actuel
        var articlesAchatsAcheteur = panier.Where(a => a.IdAcheteur == acheteur.Id);

        foreach (var articleAchete in articlesAchatsAcheteur)
        {
            var article = articles.Find(a => a.Id == articleAchete.IdArticle);
    ...
    private static void EnregistrerArticlesAchetes()
    {
        string panierJson = JsonSerializer.Serialize(panier);
        File.WriteAllText(articleAcheteFile, panierJson);
        LogAction("Articles achetés enregistrés dans le fichier articlesAchetes.json");
    }
